...
支持命令行查询：
```shell
# 查询沪市1天内(上一个交易日)涨幅5%以上的股票
python AMonitor.py -t sh -i 5 -f 1d

# 可以指定股票
python AMonitor.py -t sh -i 5 -f 1d -c 603918
类型: sh, 频率: 1d, 增幅: 5.0, 跌幅: -20.0
共监控到 1 只股票:
603918 金桥信息  25.78  28.59 10.90

# 以上后三个字段是：该时间段内的 '开盘价', '收盘价'，涨幅
```

部署后自动调度、监控：
```shell
python ScheduleMonitor.py
```
